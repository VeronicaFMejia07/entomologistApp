...
const ButtonComponent = ({title, onPress, position = 'positionRight', action}:IPropsButton) => {
    return (
        <View 
            style={[ styles.sectionTouchable, 
                (position === 'positionRight' ? styles.positionRight : styles.positionLeft), 
                
            ]}  
        >
            <TouchableNativeFeedback onPress={onPress} background={TouchableNativeFeedback.Ripple('black', true, 20)}>
                <View style={(action === 'btn' ? styles.sectionButtonSaveAndClean : styles.sectionViewButtonAdd)}>
