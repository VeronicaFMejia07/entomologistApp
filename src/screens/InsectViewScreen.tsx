        ...
        <View style={styles.sectionInsect}>
...

            {
                insectState.insects.map((item, index) => (
                    <TouchableOpacity 
                        key={index}
                        style={[styles.sectionTableRow, {backgroundColor: checkIndexIsEven(index) ? 'white' : '#EBDEF0'}]} 
                        onPress={() => navigation.navigate('InsectDescription', {insectName: item.name, insectQuantity: item.quantity})}
                    >
                        <View>
